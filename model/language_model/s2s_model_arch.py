...
from model.language_model.s2t_model_arch import SpeechQwen2ForCausalLM
...
class Speech2SConfig(Qwen2Config):
    model_type = "SpeechMedAssist"


class Speech2SQwen2ForCausalLM(SpeechQwen2ForCausalLM, GenerationWithTTS):
    config_class = Speech2SConfig
    ...
    def initialize_speech_generator(self, model_args):
        ...
        self.config.tune_speech_generator_only = getattr(model_args, "tune_speech_generator_only", True)
    ...
    def forward(
        ...
        if self.training:
            if self.config.tune_speech_generator_only:
                with torch.no_grad():
                    lm_output = super(SpeechQwen2ForCausalLM, self).forward(
                        input_ids=input_ids,
                ...
                loss = self.speech_generator(lm_output['hidden_states'][-1], labels, tgt_units)
            else:
                lm_output = super(SpeechQwen2ForCausalLM, self).forward(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                ...
                gen_loss = self.speech_generator(lm_output['hidden_states'][-1], labels, tgt_units)
                loss = lm_loss + gen_loss * self.config.ctc_loss_weight
        else:
            lm_output = super(SpeechQwen2ForCausalLM, self).forward(
                input_ids=input_ids,
                attention_mask=attention_mask,
...
AutoConfig.register("SpeechMedAssist", Speech2SConfig)
AutoModelForCausalLM.register(Speech2SConfig, Speech2SQwen2ForCausalLM)
