...
```bash
# -------- 1. 使用 HuatuoGPT2-GPT4-SFT-140K.json --------
python filter.py \
...
- Ency: [[dialog record]](./eval/single_round/dialog_record.py) to record the conversation between the model and the patient, [[eval]](./eval/single_round/evaluation.py)


### 5.2 Multi-Turn Conversation

#### 5.2.1 First get the record of the conversation between the tested model as a doctor and the virtual patient. [[dialog record]](./eval/conversation/dialog_record.py)
...
#### 5.2.2 Then evaluate the performance of the tested model. [[eval]](./eval/conversation/evaluation.py)
